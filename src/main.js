...
class AppController {
  ...
  openUserAccountModal() {
    ...
    const displayName = (u.profile && u.profile.displayName) || u.displayName || u.email.split('@')[0];
  ...
  async handleUserAvatarChange(e) {
    ...
    try {
      const uploadedUrl = await this.authRepo.updateAvatar(file, null);
      if (this.currentUser) {
  ...
  async handleEditUserProfileSubmit(e) {
    ...
    try {
      await this.authRepo.updateDisplayName(newName);
      this.updateAuthUI(this.currentUser);
      this.closeModal('userAccountModal');
  ...
  async handleDeleteAccountConfirm(e) {
    ...
    try {
      await this.authRepo.deleteAccountCascade(isGoogleAuth ? null : pass);
